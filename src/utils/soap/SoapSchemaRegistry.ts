...
export async function loadSoapSchema(schemaPath: string): Promise<object> {
...
export async function getSoapSchemaValidator(
  ...
  if (typeof schemaOrPath === 'string') {
    ...
    const schema = await loadSoapSchema(abs);
